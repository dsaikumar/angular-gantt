...
export class GanttColumnsManager {
  ...
  constructor (gantt) {
    ...
    this.gantt.api.data.on.load(this.gantt.$scope, () => {
    ...
    this.gantt.api.data.on.remove(this.gantt.$scope, () => {
      this.gantt.rowsManager.sortRows();
    });
   ...
   * @returns {GanttColumn}
   */
  getColumnByDate(date, disableExpand) {
    if (!disableExpand) {
   ...
   * @returns {GanttColumn}
   */
  getColumnByPosition(x, disableExpand) {
    if (!disableExpand) {
  ...
  updateVisibleColumns(includeViews) {
    ...
    if (limitThreshold === undefined || limitThreshold > 0 && this.columns.length >= limitThreshold) {
      ...
      for (i = 0; i < this.headers.length; i++) {
        this.visibleHeaders.push(GanttColumnsManager.$filter('ganttColumnLimit')(this.headers[i], this.gantt));
      }
    } else {
...
export default function (
  GanttColumnGenerator: GanttColumnGenerator,
