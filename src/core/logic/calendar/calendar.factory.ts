...
export class DateFrame {
  ...
  dateMatch = function (date: moment.Moment) {
    if (this.evaluator) {
...
export class GanttCalendar {
   ...
   */
  removeTimeFrames = function (timeFrames: string[]) {
    for (let name in timeFrames) {
   ...
   */
  registerTimeFrameMappings = function (mappings: {(date: moment.Moment): any}) {
   ...
   */
  removeDateFrames = function (dateFrames: string[]) {
    for (let name in dateFrames) {
